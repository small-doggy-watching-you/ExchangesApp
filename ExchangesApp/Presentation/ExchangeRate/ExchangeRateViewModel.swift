...
final class ExchangeRateViewModel: ViewModelProtocol {
  ...
  private func loadData() {
    ...
    useCase.loadCurrencyNames { [weak self] result in
  ...
  private func fetchRates() {
    useCase.fetchExchangeRates { [weak self] result in
      guard let self else { return }
      DispatchQueue.main.async {
        switch result {
        case .success(var items):
          self.useCase.syncFavorites(to: &items)
  ...
  private func toggleFavorite(code: String) {
    ...
    useCase.updateFavoriteStatus(for: items[index])
    state.items = items
